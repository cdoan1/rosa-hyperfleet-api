...
func (r *eventRecorder) waitForType(t *testing.T, typ string, timeout time.Duration) recordedEvent { //nolint:unparam
	t.Helper()
	deadline := time.After(timeout)
	for {
		select {
...
//nolint:unparam // test helpers are designed for reuse
...
// WaitForCacheSync must honor its context: a canceled context has to unblock
...
func TestCache_WaitForCacheSyncHonorsContext(t *testing.T) {
	...
	select {
	case synced := <-done:
		assert.False(t, synced, "WaitForCacheSync should report failure on canceled context")
