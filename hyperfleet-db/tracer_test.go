...
func newTracedPool(t *testing.T, threshold time.Duration, logger *slog.Logger) *pgxpool.Pool {
	...
	config.ConnConfig.Tracer = hyperfleetdb.NewSlowQueryTracer(threshold, logger)
...
func TestSlowQueryTracer_PIDMatchesBackend(t *testing.T) {
	...
	require.NoError(t, err)
	config.MaxConns = 1
	config.ConnConfig.Tracer = hyperfleetdb.NewSlowQueryTracer(500*time.Millisecond, logger)
