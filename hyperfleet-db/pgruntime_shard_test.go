	...
	*out = *in
	...
	if in.Items != nil {
		...
		for i := range in.Items {
			...
			in.Items[i].DeepCopyInto(&out.Items[i].ObjectMeta)
		}
	}
	return out
}
...
func init() {
	b := &runtimescheme.Builder{GroupVersion: testGV} //nolint:staticcheck
...
func newShardedManager(t *testing.T, shard *hyperfleetdb.ShardConfig) ctrl.Manager {
	t.Helper()
	conn := sharedDB.Connect(t)
...
func TestShardConfig_Validation(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Mod zero",
			shard:       &hyperfleetdb.ShardConfig{Mod: 0, Owned: []int{0}},
			errContains: "Mod must be > 0",
		},
		{
			name:        "Mod negative",
			shard:       &hyperfleetdb.ShardConfig{Mod: -1, Owned: []int{0}},
			errContains: "Mod must be > 0",
		},
		{
			name:        "empty Owned",
			shard:       &hyperfleetdb.ShardConfig{Mod: 2, Owned: []int{}},
			errContains: "Owned must be non-empty",
		},
		{
			name:        "Owned out of range",
			shard:       &hyperfleetdb.ShardConfig{Mod: 2, Owned: []int{3}},
			errContains: "out of range",
		},
		{
			name:        "duplicate Owned",
			shard:       &hyperfleetdb.ShardConfig{Mod: 4, Owned: []int{1, 1}},
			errContains: "duplicate",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := hyperfleetdb.NewManager(hyperfleetdb.Options{
				Scheme: testScheme,
				DSN:    sharedDB.ConnStr,
				Shard:  tt.shard,
				Logger: logr.Discard(),
	...
	t.Run("valid shard accepted", func(t *testing.T) {
		mgr := newShardedManager(t, &hyperfleetdb.ShardConfig{
			Mod: 2, Owned: []int{0},
		})
		assert.NotNil(t, mgr)
	})
...
func TestShardedManager_TwoReplicas(t *testing.T) {
	...
	mgr0, err := hyperfleetdb.NewManager(hyperfleetdb.Options{
		Scheme: testScheme,
		DSN:    sharedDB.ConnStr,
		Shard:  &hyperfleetdb.ShardConfig{Mod: mod, Owned: []int{0}},
		Logger: logr.Discard(),
	})
	require.NoError(t, err)

	mgr1, err := hyperfleetdb.NewManager(hyperfleetdb.Options{
		Scheme: testScheme,
		DSN:    sharedDB.ConnStr,
		Shard:  &hyperfleetdb.ShardConfig{Mod: mod, Owned: []int{1}},
		Logger: logr.Discard(),
	})
	require.NoError(t, err)
