...
func TestR5_Toxi_AmbiguousCommit_ResetPeer(t *testing.T) {
	...
	err = verifyConn.QueryRow(ctx,
		`SELECT count(*) FROM kubernetes_resources`+
			` WHERE gvk = 'apps/v1/Deployment'`+
			` AND namespace = 'default' AND name = 'toxi-ambig'`,
	).Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count, "exactly one resource row must exist")

	// Clean up: remove toxic if it's still there, re-enable proxy
	_ = pdb.Proxy.RemoveToxic("commit-kill")
	_ = pdb.Proxy.Enable()
	_ = proxiedWriterConn.Close(ctx)
}
...
func (h *toxiCommitCutHook) AfterSuppressionCheck(_ context.Context, _ pgx.Tx, _ bool) error {
	return nil
}
