...
func TestR7_CompactionVsSlowWatcher(t *testing.T) {
	...
	_, err := backdateConn.Exec(ctx,
		`UPDATE kubernetes_resources SET updated_at = deletion_timestamp`+
			` WHERE name LIKE 'compact-victim-%'`)
	require.NoError(t, err)
	_ = backdateConn.Close(context.Background())
...
func TestR7_CompactionBoundary_Exact(t *testing.T) {
	...
	_, err = backdateConn2.Exec(ctx,
		`UPDATE kubernetes_resources SET updated_at = deletion_timestamp`+
			` WHERE name = 'compact-exact'`)
	require.NoError(t, err)
	_ = backdateConn2.Close(context.Background())
