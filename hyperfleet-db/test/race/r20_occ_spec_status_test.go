...
func TestR20_ConcurrentSpecStatusRace(t *testing.T) {
	...
	w0 := newWriter(t, nil)
	createReq := makeWriteReq("apps/v1/Deployment", "default", "r20")
	...
	// txid_stamp reflects spec update only. Status rollback.
	conn := freshConn(t)
	var txidStamp int64
	...
	require.NoError(t, err)
	assert.Equal(t, int64(resA.result.Txid), txidStamp)
