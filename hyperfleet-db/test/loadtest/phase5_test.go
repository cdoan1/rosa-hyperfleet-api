...
func TestPhase5_PollCostAndDeliveryLatency(t *testing.T) { //nolint:gocyclo
	if testing.Short() {
	...
	probeCancel()
	<-probeDone
	_ = probeConn.Close(context.Background())
	...
	for i := range numWatchers {
		<-idleDones[i]
		_ = idleConns[i].Close(context.Background())
	}
	...
	for i := range numWatchers {
		<-dbDones[i]
		_ = dbPollConns[i].Close(context.Background())
		_ = dbListenConns[i].Close(context.Background())
	}

	// Compute delivery latencies
	dbLatencies := make([]time.Duration, 0, len(deliveries))
	for _, d := range deliveries {
		dbLatencies = append(dbLatencies, d.recvTime.Sub(d.writeTime))
	}
	slices.Sort(dbLatencies)
	...
	_ = lossWriteConn.Close(context.Background())
	lossCollectWg.Wait()

	lossCancel()
	for i := range numWatchers {
		<-lossDones[i]
		_ = lossConns[i].Close(context.Background())
	}

	lossLatencies := make([]time.Duration, 0, len(lossDeliveries))
	for _, d := range lossDeliveries {
		lossLatencies = append(lossLatencies, d.recvTime.Sub(d.writeTime))
	}
	slices.Sort(lossLatencies)
	...
	verCancel()
	<-verDone
	_ = verifyConn.Close(context.Background())
