...
func TestCeiling_WorkerScaling(t *testing.T) {
	...
	for _, workers := range workerCounts {
		...
		t.Run(name, func(t *testing.T) {
			...
			slices.SortFunc(combined, func(a, b time.Duration) int { return int(a - b) })
		...
		t.Run(name, func(t *testing.T) {
			...
			if enableVerifiers {
				...
				for _, ve := range verifiers {
					...
					_ = ve.conn.Close(context.Background())
					_ = ve.canary.Close(context.Background())
				}
			}
			...
			slices.SortFunc(allLatencies, func(a, b time.Duration) int { return int(a - b) })
