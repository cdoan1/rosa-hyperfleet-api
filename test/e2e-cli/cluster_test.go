...
var _ = Describe("ROSACTL CLI E2E Tests", Ordered, func() {
	...
	// Delete cluster-vpc: issue the delete command without waiting for completion.
	// The CloudFormation stack will eventually delete successfully.
	It("should be able to delete the cluster-vpc", Label("vpc-delete", "cleanup"), func() {
		// Check if the cluster-vpc exists before attempting delete
		cmd := exec.Command(ROSACTL_BIN, "cluster-vpc", "list", "--region", region)
		cmd.Env = append(os.Environ(), customerEnv()...)
		output, err := cmd.CombinedOutput()
		if err != nil {
		...
		// Issue the delete command without waiting for completion
		GinkgoWriter.Printf("Issuing cluster-vpc delete: %s\n", clusterName)
		cmd = exec.Command(ROSACTL_BIN, "cluster-vpc", "delete", clusterName, "--region", region)
		cmd.Env = append(os.Environ(), customerEnv()...)
		output, err = cmd.CombinedOutput()
		if err != nil {
			GinkgoWriter.Printf("cluster-vpc delete command failed: %v\nOutput:\n%s\n", err, string(output))
			GinkgoWriter.Printf("Note: CloudFormation stack will eventually delete. Continuing with cleanup.\n")
		} else {
			GinkgoWriter.Printf("cluster-vpc delete command issued: %s\n", clusterName)
		}
	})
