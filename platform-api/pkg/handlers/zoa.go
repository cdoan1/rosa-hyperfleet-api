...
func (h *ZoaHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if v := query.Get("dry_run"); v == "true" { //nolint:goconst // "true" is a standard string literal
		b := true
		filter.DryRun = &b
	} else if v == "false" {
		b := false
...
func (h *ZoaHandler) checkMaxConcurrent(ctx context.Context, accountID, targetCluster string, maxConcurrent int) error {
	...
	running, err := h.store.List(ctx, accountID, maxConcurrent+1, filter)
	if err != nil {
	...
	pending, err := h.store.List(ctx, accountID, maxConcurrent+1, pendingFilter)
	if err != nil {
