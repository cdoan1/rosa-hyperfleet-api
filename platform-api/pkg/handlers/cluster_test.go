...
func TestClusterHandler_Create_ConcurrentOidcConfigCollision(t *testing.T) {
	...
	if created != 1 || idExhausted != 1 {
		t.Fatalf("expected exactly one 201 and one 500 (ErrClusterCreateIDExhausted) for concurrent creates referencing the same oidcConfigId, got codes %v", codes)
	}
}
