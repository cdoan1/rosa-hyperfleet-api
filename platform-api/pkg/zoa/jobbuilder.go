...
func buildRBACResources(tmpl *TATemplate, ctx RenderContext, saName string, labels map[string]string) ([]hyperfleetv1alpha1.ResourceTemplate, error) {
	...
	rules := make([]map[string]any, 0, len(tmpl.RBAC.Rules))
...
func buildRunnerJob(tmpl *TATemplate, ctx RenderContext, saName string, labels map[string]string) (hyperfleetv1alpha1.ResourceTemplate, error) {
	...
	envVars := make([]map[string]any, 0, 10+len(tmpl.Params))
	envVars = append(envVars,
		map[string]any{"name": "RUN_ID", "value": ctx.ExecID},
		map[string]any{"name": "JOB_NAME", "value": jobName},
		map[string]any{"name": "JOB_NAMESPACE", "value": ctx.Namespace},
		map[string]any{"name": "CLUSTER_ID", "value": ctx.TargetCluster},
		map[string]any{"name": "ACTION_NAME", "value": ctx.ActionName},
		map[string]any{"name": "OPERATOR", "value": ctx.Operator},
		map[string]any{"name": "REVISION", "value": ctx.Revision},
		map[string]any{"name": "TYPE", "value": ctx.Type},
		map[string]any{"name": "SCOPE", "value": ctx.Scope},
		map[string]any{"name": "OUTPUT_CONFIGMAP", "value": "zoa-output-" + ctx.ExecID},
	)
			...
			"template": map[string]any{
				...
				"spec": map[string]any{
					...
					"volumes": []map[string]any{
						{"name": "artifacts", "emptyDir": map[string]any{}},
						{
							"name": "zoa-scripts",
...
func validateSecretsPolicy(tmpl *TATemplate, ctx RenderContext) error {
	...
	if slices.ContainsFunc(tmpl.RBAC.Rules, ruleGrantsSecrets) {
...
func toResourceTemplate(resource string, obj any, watch bool) (hyperfleetv1alpha1.ResourceTemplate, error) {
