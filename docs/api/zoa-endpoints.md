...
Execution created and dispatched via Manifest.
...
Indicates Manifest creation on hyperfleet-db failed. The execution record exists in DynamoDB with `status: failed`.
...
| 500 | `render-error` | Manifest generation failed |
| 500 | `dry-run-error` | `dry_run_action` references unknown TA |
| 502 | `dispatch-error` | Manifest creation on hyperfleet-db failed |
...
- `pending`: DynamoDB record created, Manifest CR created on hyperfleet-db
...
| `manifestWorkName` | String | — | Manifest CR name |
