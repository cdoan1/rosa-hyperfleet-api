...
var _ = Describe("Manifest lifecycle", func() {
			...
			toUpdate.Spec.Resources[3].Content = runtime.RawExtension{
				Raw: []byte(
					`{"apiVersion":"batch/v1","kind":"Job",` +
						...
						`"containers":[{"name":"runner","image":"registry.example.com/e2e-runner:v2"}],` +
						`"restartPolicy":"Never"}}}}`,
				),
			}
	...
	It("should write delete ApplyDesires and clean up when Manifest is deleted", func() {
		...
		Eventually(func() error {
			nn := types.NamespacedName{Namespace: testNS, Name: manifestName}
			return k8sClient.Get(ctx, nn, &hyperfleetv1alpha1.Manifest{})
		}).ShouldNot(Succeed())
	})
	...
	It("should not collide when two Manifests deploy the same resource", func() {
		...
		hfmB := newTestManifest("e2e-monitoring-b")
