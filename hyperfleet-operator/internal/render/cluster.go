...
// oidcSigningKeyExternal must be true only when cluster.Spec.OidcConfigID is
// set AND that config is type=unmanaged, i.e. the operator actually stores a
// signing key for it in Secrets Manager (see OidcConfigReconciler). Managed
// configs don't yet provision key material, so callers must pass false for
// them or HyperShift will wait forever on a Secret that's never created.
func ClusterResources(cluster *hyperfleetv1alpha1.Cluster, oidcSigningKeyExternal bool, rcfg RegionalConfig) ([]Resource, error) {
	...
	if oidcSigningKeyExternal {
		resources = append(resources, oidcSigningKeySecret(cluster.Spec.OidcConfigID, clusterID, ns))
	}
...
func oidcSigningKeySecretStorePath(oidcConfigID string) string {
	return oidc.SecretName(oidcConfigID)
}
...
func oidcSigningKeySecret(oidcConfigID, clusterID, ns string) Resource {
	return Resource{
		...
		Object: &ExternalSecret{
			...
			Spec: ExternalSecretSpec{
				...
				Data: []ExternalSecretDataEntry{
					{
						SecretKey: "key",
						RemoteRef: ExternalRemoteRef{Key: oidcSigningKeySecretStorePath(oidcConfigID)},
					},
				},
			},
		},
