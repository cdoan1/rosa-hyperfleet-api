...
// status by inspecting Manifest CR status on hyperfleet-db.
...
func (r *Reconciler) reconcileExecution(ctx context.Context, exec *Execution) {
	...
	hfm, err := r.db.GetManifest(ctx, JobNamespace, exec.ManifestWorkName)
	if err != nil {
		if hyperfleetdb.IsNotFound(err) {
			return
		}
		r.logger.Error("failed to get manifest from hyperfleet-db",
...
// deleteManifest removes the Manifest from hyperfleet-db. Returns nil on success or
...
func (r *Reconciler) deleteManifest(ctx context.Context, exec *Execution) error {
	err := r.db.DeleteManifest(ctx, JobNamespace, exec.ManifestWorkName)
	if err != nil {
		if hyperfleetdb.IsNotFound(err) {
