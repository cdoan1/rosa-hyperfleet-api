...
func New(cfg *config.Config, dbClient *hyperfleetdb.Client, logger *slog.Logger) (*Server, error) {
	...
	clusterHandler := apphandlers.NewClusterHandler(dbClient, cfg.Regional.OIDCIssuerBaseURL, fieldValidator, logger)
	nodePoolHandler := apphandlers.NewNodePoolHandler(dbClient, fieldValidator, logger)
