...
func newFakeDB() *hyperfleetdb.Client {
	scheme := runtime.NewScheme()
	_ = hyperfleetv1alpha1.AddToScheme(scheme)
	return hyperfleetdb.NewClientFrom(fake.NewClientBuilder().WithScheme(scheme).Build(), testZoaLogger())
}
...
func newTestZoaHandler(t *testing.T, store zoa.ExecutionStore, fdb *hyperfleetdb.Client) *ZoaHandler {
	t.Helper()
...
func TestZoaHandler_Get_Found(t *testing.T) {
	...
	handler := newTestZoaHandler(t, store, newFakeDB())
...
func TestZoaHandler_Describe(t *testing.T) {
	handler := newTestZoaHandler(t, &mockExecutionStore{}, newFakeDB())
...
func TestZoaHandler_Get_IncludeLogs(t *testing.T) {
	...
	s3Mock := &mockS3Client{}
	handler := NewZoaHandler(store, testTemplateRegistry(t), newFakeDB(), s3Mock, ZoaConfig{
		BucketName: "test-bucket",
		JobConfig:  testJobConfig(),
	}, testZoaLogger())
...
func TestZoaHandler_AuditList(t *testing.T) {
	...
	handler := NewZoaHandler(&mockExecutionStore{}, testTemplateRegistry(t), newFakeDB(), &mockS3Client{}, ZoaConfig{
		BucketName: "test-bucket",
		JobConfig:  testJobConfig(),
